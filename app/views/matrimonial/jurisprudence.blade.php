		 	 ...
		 	 <div class="arrets">
			 	 ...
			 	 <div id="analyses" class="cat clear analyse">			 	 
					<div class="details large-3 columns">
						<img src="{{ asset('/images/matrimonial/categories/analyse.jpg') }}" alt="" width="140" height="140" />
						<h4>Analyse</h4>
					</div>
						...
						@foreach($allanalyses as $analyse)
							<a name="analyse-{{ $analyse->id }}"></a>
								...
								<div class="arret analyse c{{ $analyse->categories }} {{ $allcat }}">
									...
									<ul class="liste-arrets arrets-internal-links">
										@if(!empty($anarr))
											@foreach($anarr as $arr)
												<li>{{ link_to('matrimonial/jurisprudence#a-'.$arr['id'] , $arr->reference ) }} du {{ $arr->pub_date->formatLocalized('%e %B %Y') }}</li>
											@endforeach 
										@endif
									</ul>							
									...
									<p><a href="uploads/tx_matrimonialarrets/{$analyse.file}" target="_blank">Télécharger cette analyse en PDF</a></p>
									@endif
								</div>
				 ...
				 <!-- liste des arrets -->
				 <div class="cat clear matrimonial">
						<div class="liste">
							...
							@foreach($arrets as $arret)
								...
								<div class="arret {{ $allarrcat }} y{{ $arret->pub_date->year }} clear">
									...
									@foreach($categories_arret as $cat)							
										<div class="details">					
											<img src="{{ asset('/images/matrimonial/categories/'.$cat['image']) }}" alt="{{ $cat['title'] }}" width="140" height="140" />
